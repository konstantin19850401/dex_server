...
class Toolbox {
    ...
    async getDocumentStatuses() {
        ...
        let statuses = await this.sqlRequest('skyline', `SELECT * FROM dict_doc_statuses WHERE status = '1'`);
    ...
    // получение случайного хэш-значения
    getHash(leng) {
    ...
    // получение закодированной строки в md5
    criptoHashMD5(data) {
        return crypto.MD5(data, 'appadapters').toString();
    }
    // есть ли дирректория
    async ifIssetDirrectory(path) {
        return new Promise((resolve, reject)=> {
    ...
    // сжатие картинки
    async compressionImage(path) {
        return new Promise((resolve, reject)=> {
            let bitmap = fs.readFileSync(path);
            let base64String = Buffer(bitmap).toString('base64');
            exec(`jpegoptim --strip-all --size=49k --all-progressive -Pptm90 ${path}`, function(error, stdout, stderr) {
                console.log(error);
    ...
    // копирование объекта 
    // передача откидываемого поля имеет вид "поле.поле.поле.поле" 
    copyObjectWithoutOneField(object, search) {
        let newObject = {};
        let field = [];
        function nextProp(object, newObject) {
            for (let key in object) {
                field.push(key);
                if (typeof object[key] === 'object') {
                    if (typeof newObject[key] === 'undefined') newObject[key] = {};
                    nextProp(object[key], newObject[key]);
                } else {
                    let f = field.join('.');
                    if (f != search) newObject[key] = object[key];
                }
                field.pop();
            }
        }
        nextProp(object, newObject);
        return newObject;
    }
    // формирование объекта перед сохранением документа в базе
    async preparationContract(user, contract, contractId, digest, criticalsFields, base) {
        let moment = this.getMoment();
        let date = moment(new Date(), 'DD.MM.YYYY');
        let rand = 0 - 0.5 + Math.random() * (99999999 - 0 + 1);
        let strD8 = Math.round(rand).toString();
        while (strD8.length < 8) strD8 = "0"+strD8;

        let NEWROW = {};
        NEWROW.locked = '';
        NEWROW.locktime = '';
        NEWROW.userid = user;
        NEWROW.status = contract.status;
        NEWROW.signature = `${date.format('YYYYMMDDhhmmss')}146${strD8}`;
        NEWROW.jdocdate = `${date.format('YYYYMMDD')}120000000`;
        NEWROW.unitid = contract.unitid;
        NEWROW.docid = contractId;
        NEWROW.digest = digest;
        NEWROW.importhash = "";

        let DOC = {};
        DOC.Document = {};
        DOC.Document.$ = {};
        DOC.Document.$.ID = contractId;
        for (let key in contract) {
            DOC.Document[key] = contract[key];
        }
        let xml2js = this.getXml2js();
        let builder = new xml2js.Builder();
        let xmlData = builder.buildObject(DOC);
        NEWROW.data = xmlData;

        let jour = {};
        let criticals = 0;
        let sqlCriticals;
        let rows = await this.sqlRequest(base, `SELECT * FROM criticals WHERE cname = 'ICC' AND cvalue= '${contract.ICC}' AND signature <> ${NEWROW.signature}`);
        if (rows.length > 0) criticals++;
        if (criticalsFields.length == 2) {
            rows = await this.sqlRequest(base, `SELECT * FROM criticals WHERE cname = 'MSISDN' AND cvalue= '${contract.MSISDN}' AND signature <> ${NEWROW.signature}`);
            if (rows.length > 0) criticals++; 
        } 
        jour.lournal = {};
        jour.record = {};
        jour.record.$ = {};
        jour.record.$.time = date.format('HH:MM:ss DD.MM.YYYY');
        jour.record.subdata = [`Дайджест: ${NEWROW.digest}`];
        let draft = false;
        let statuses =  await this.getDocumentStatuses();
        if (criticalsFields.length == 2 && criticals == 2 || criticalsFields.length == 1 && criticals == 1) draft = true;
        if (draft) {
            console.log("будет черновиком");
            NEWROW.status = statuses.DOCUMENT_DRAFT;
            jour.record.text = ["Новый документ добавлен в журнал посредством web-app. Выявлены совпадения по базе. Статус документа понижен."];
        } else {
            console.log("не будет черновиком");
            jour.record.text = ["Новый документ добавлен в журнал посредством web-app\тПривет"];
        }
        console.log('jour=> ', jour);
        let xmlJournal = builder.buildObject(jour);
        NEWROW.journal = xmlJournal;
